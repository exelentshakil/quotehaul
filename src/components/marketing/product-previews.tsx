  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-6 shadow-popover">
      <div className="mb-4 grid grid-cols-3 gap-3">
        <StatCard label="Open leads" value="7" className="p-4" />
        <StatCard label="Booked" value="3" className="p-4" />
        <StatCard label="Pipeline" value="£4.6k" className="p-4" />
      </div>
      <div className="space-y-2.5">
        {cards.map((c) => (
          <div key={c.name} className="rounded-lg border border-border bg-background p-3.5">
...
export function MessagingPreview() {
  return (
    <div className="rounded-2xl border border-border bg-card shadow-popover">
      ...
      <div className="border-t border-border p-4">
        <div className="flex items-center gap-2 rounded-md border border-input bg-background px-3 py-2.5 text-sm text-muted-foreground">
          <Send className="h-3.5 w-3.5" /> Reply straight from your email inbox
        </div>
      </div>
    </div>
  );
    ...
    <div className="rounded-2xl border border-border bg-card p-6 shadow-popover">
      ...
      <div className="mt-3 grid grid-cols-3 gap-2 text-xs">
        {["Studio", "1 bed", "2 bed"].map((s, i) => (
          <div key={s} className={`rounded-md border px-3 py-2.5 text-center ${i === 2 ? "border-primary bg-primary text-primary-foreground" : "border-border"}`}>{s}</div>
        ))}
      </div>
      <div className="mt-4 rounded-lg bg-muted/60 p-4 text-center">
        ...
        <p className="font-display text-2xl font-bold">£420–£510</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-6 shadow-popover">
      ...
      <div className="space-y-2">
        {days.map((d) => (
          <div key={d.d} className="flex items-center justify-between rounded-md border border-border px-3.5 py-2.5 text-xs">
...
export function ScorePreview() {
  return (
    <div className="rounded-2xl border border-border bg-card p-6 shadow-popover">
      ...
      <p className="mt-2 font-display text-4xl font-bold text-success">82<span className="font-sans text-base font-normal text-muted-foreground">/100</span></p>
      <div className="mt-3 rounded-md bg-muted/60 p-3.5 text-xs text-muted-foreground">
